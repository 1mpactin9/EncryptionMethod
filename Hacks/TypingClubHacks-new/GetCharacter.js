...
function getTargetCharacters() {
  const els = Array.from(document.querySelectorAll('.token_unit'));
  const chrs = els
    .map(el => {
      let text = el.textContent.trim();
      if (text === '' && el.querySelector('br')) {
        return '\n';
      }
      return text || ' ';
    })
...
function getActiveCharElement() {
    // Assuming the active character is the first .token_unit in the ._fcs token
    const fcsToken = document.querySelector('.token._fcs');
    if (fcsToken) {
        return fcsToken.querySelector('.token_unit');
    }
    // Fallback: first token_unit
    return document.querySelector('.token_unit');
}
