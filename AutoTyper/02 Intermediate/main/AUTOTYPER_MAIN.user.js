// ==UserScript==
// @name         TypingClub AutoTyper (Combined Optimized)
// @namespace    https://github.com/EncryptionMethod/AutoTyper
// @version      2.0.0
// @description  Auto-complete TypingClub lessons with human-like WPM, accuracy simulation, and smart delay compensation
...
(function () {
  ...
  // ============================================================
  // SPEED PATTERN ENGINE + SMART COMPENSATION
  // ============================================================
  ...
  function getMinDelay() {
    return 12000 / config.maxWPM; // faster WPM = smaller delay
  }

  function getMaxDelay() {
    return 12000 / config.minWPM; // slower WPM = larger delay
  }
  ...
  // Get smart compensated delay after applying speed pattern
  function getCompensatedDelay(baseDelay, charsLeft) {
    const now = Date.now();
    const timeRemaining = state.targetEndTime - now;

    if (timeRemaining <= 0) return getMinDelay();

    const neededDelay = timeRemaining / charsLeft;
    // Add 10% human jitter
    const jitter = neededDelay * (0.9 + Math.random() * 0.2);
    // Clamp to min/max WPM bounds
    return Math.max(getMinDelay(), Math.min(jitter, getMaxDelay()));
  }

  // ============================================================
  // TYPING ENGINE - Robust implementation from InitialScript
  // ============================================================

  function getInput() {
    return document.querySelector('input[aria-hidden="true"]') || document.activeElement;
  }

  // Robust character extraction with special handling
  ...
  // Complete forceType with full event sequence
  async function forceType(char) {
  ...
  function scrapeMenuLessons() {
    // Try multiple approaches to find lessons - works for both public and Sportal layouts
    let lessons = [];
    let globalIndex = 0;

    // Approach 1: Look for all box-container anywhere (contains individual lesson boxes)
    const allBoxContainers = document.querySelectorAll('.box-container');
    if (allBoxContainers.length > 0) {
      Logger.info('Scraper', `Found ${allBoxContainers.length} total boxes via broad search`);
      ...
      Logger.info('Scraper', `Scanned ${lessons.length} total lessons`);
      return lessons;
    }

    // Approach 2: Original hardcoded container (for backward compatibility with public layout)
    const container = document.querySelector('body > div:nth-child(1) > div:nth-child(1) > div:nth-child(3) > div > div:nth-child(3) > div');
    if (!container) {
      Logger.error('Scraper', 'No menu container found (tried both broad and specific selectors)');
      return [];
    }
  ...
  async function executeLesson(lessonData, signal) {
    ...
    pattern.reset();
    const baseBaseDelay = getBaseDelay();
    const chars = extractChars();
    const totalChars = chars.length;
    state.lessonStartTime = Date.now();

    // Calculate target end time for smart compensation
    const targetDurationMs = (totalChars * 12000) / config.targetWPM;
    state.targetEndTime = Date.now() + targetDurationMs;

    Logger.info('Typing', `Typing ${totalChars} characters at ~${config.targetWPM} WPM (target duration: ${Math.round(targetDurationMs / 1000)}s)`);
    ...
    for (let i = 0; i < totalChars; i++) {
      ...
      if (detectPage() === 'RESULTS') {
        Logger.ok('Typing', `Lesson completed early at char ${i}/${totalChars}`);
        return true;
      }

      const charsLeft = totalChars - i;
      const baseDelay = pattern.getNext(baseBaseDelay, i, totalChars);
      const finalDelay = getCompensatedDelay(baseDelay, charsLeft);

      // Decide: normal char or fake error
      if (shouldFakeError()) {
        // Fake error: type wrong → backspace → correct
        const wrongChar = getRandomErrorChar();
        await forceType(wrongChar);
        await sleep(getCompensatedDelay(finalDelay, charsLeft + 2));
        await forceType("Backspace");
        await sleep(getCompensatedDelay(finalDelay, charsLeft + 1));
      }

      // Type the correct character
      const trueChar = chars[i];
      await forceType(trueChar);
      await sleep(finalDelay);
    }

    Logger.ok('Typing', `Finished typing ${totalChars} characters`);
  ...
  // ============================================================
  // FLOATING UI - Catppuccin Mocha Theme
  // ============================================================
  ...
  const UI = {
    ...
    createPanel() {
      ...
      this.panel.innerHTML = `
        ...
        <div id="at-body">
          ...
          <div style="margin-bottom:10px;">
            <label style="display:flex;justify-content:space-between;color:#a6adc8;">
              Min WPM: <span id="at-minwpm-val" style="color:#89b4fa;">${config.minWPM}</span>
            </label>
            <input type="range" id="at-minwpm" min="20" max="200" value="${config.minWPM}"
              style="width:100%;accent-color:#89b4fa;">
          </div>
          ...
          <div style="margin-bottom:10px;">
            <label style="display:flex;justify-content:space-between;color:#a6adc8;">
              Max WPM: <span id="at-maxwpm-val" style="color:#89b4fa;">${config.maxWPM}</span>
            </label>
            <input type="range" id="at-maxwpm" min="20" max="200" value="${config.maxWPM}"
              style="width:100%;accent-color:#89b4fa;">
          </div>
    ...
    bindEvents() {
      ...
      // Min WPM slider
      const minwpmSlider = document.getElementById('at-minwpm');
      minwpmSlider.addEventListener('input', () => {
        config.minWPM = parseInt(minwpmSlider.value);
        document.getElementById('at-minwpm-val').textContent = config.minWPM;
      });

      // Max WPM slider
      const maxwpmSlider = document.getElementById('at-maxwpm');
      maxwpmSlider.addEventListener('input', () => {
        config.maxWPM = parseInt(maxwpmSlider.value);
        document.getElementById('at-maxwpm-val').textContent = config.maxWPM;
      });
    ...
    updateStats(results) {
      ...
      if (state.currentLessonIndex >= 0) {
        document.getElementById('at-current-lesson').textContent = `Current: #${state.currentLessonIndex}`;
      } else {
        document.getElementById('at-current-lesson').textContent = 'Current: None';
      }
    },
  ...
  setTimeout(() => {
    UI.createPanel();
    Logger.info('Init', 'AutoTyper v2.0.0 loaded. Click the 🤖 button to open controls.');
  }, 1000);
